...
contract TradingPoolHelpers {
    ...
    function simulateBuy(
        ...
        DataTypes.LiquidityPair721[]
            memory liquidityPairsData = new DataTypes.LiquidityPair721[](
                nftIds.length
            );
        ...
        // scope vars to Avoid stack too deep errors
        {
            ...
            for (uint i = 0; i < nftIds.length; i++) {
                ...
                require(
                    IERC721(ITradingPool721(pool).getNFT()).ownerOf(
                        nftIds[i]
                    ) == pool,
                    "TP:B:NOT_OWNER"
                );
                lpIndex = ITradingPool721(pool).nftToLp(nftIds[i]);
                ...
                if (!lpFound) {
                    ...
                    lp = ITradingPool721(pool).getLP(lpIndex);
    ...
    {
        uint256 lpCount = ITradingPool721(pool).getLpCount();
        ...
        for (uint i = 0; i < lpCount; i++) {
            lp = ITradingPool721(pool).getLP(i);
        ...
        for (uint i = 0; i < validLiquidityPairsCount; i++) {
            lp = ITradingPool721(pool).getLP(validLiquidityPairs[i]);
        ...
        DataTypes.LiquidityPair721[]
            memory sellLiquidityPairsData = new DataTypes.LiquidityPair721[](
                amount
            );
        sellLiquidityPairsData[0] = ITradingPool721(pool).getLP(
