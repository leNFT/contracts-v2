...
contract TradingGauge is IGauge, ERC165, ERC721Holder, ReentrancyGuard {
    ...
    function claim() external returns (uint256 amountToClaim) {
        ...
        // Update the user next claimable epoch
    ...
    function deposit(uint256 lpId) external nonReentrant {
        DataTypes.TokenStandard nftType = ITradingPool(_lpToken).getNFTType();

        uint256 depositLpValue;
        if (nftType == DataTypes.TokenStandard.ERC1155) {
            // Get LP token
            DataTypes.LiquidityPair1155 memory lp = ITradingPool1155(_lpToken)
                .getLP(lpId);
            ...
            // Add token value
            depositLpValue = calculateLpValue(
                lp.nftAmount,
                lp.tokenAmount,
                lp.spotPrice
            );
            ...
            // Get LP token
            DataTypes.LiquidityPair721 memory lp = ITradingPool721(_lpToken)
                .getLP(lpId);
            // Only Trade type LPs can be staked
            require(
                lp.lpType == DataTypes.LPType.Trade ||
