...
contract WETHGateway is ERC721Holder {
    ...
    function depositTradingPool721(
        address pool,
        ...
        ITradingPool721(pool).addLiquidity(
            msg.sender,
            lpType,
            nftIds,
            msg.value,
    ...
    function withdrawTradingPool721(
        address pool,
        uint256 lpId
        ...
        DataTypes.LiquidityPair721 memory lp = ITradingPool721(pool).getLP(
            lpId
        );

        // Remove liquidity
        ITradingPool721(pool).removeLiquidity(lpId);
    ...
    function withdrawBatchTradingPool721(
        address pool,
        ...
        ITradingPool721(pool).removeLiquidityBatch(lpIds);
    ...
    function buy721(
        ...
        uint256 finalPrice = ITradingPool721(pool).buy(
            msg.sender,
            nftIds,
            maximumPrice
        );
    ...
    function sell721(
        ...
        // Sell NFTs
        uint256 finalPrice = ITradingPool721(pool).sell(
            address(this),
            nftIds,
            liquidityPairs,
            minimumPrice
