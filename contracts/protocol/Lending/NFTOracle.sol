...
contract NFTOracle is INFTOracle, Ownable, Trustus {
    ...
    function getTokens721ETHPrice(
        ...
        return _getTokens721ETHPrice(collection, tokenIds, request, packet);
    }
    ...
    /// @param packet The packet relayed by the caller
    /// @return The ETH value of the specified tokens
    function _getTokens721ETHPrice(
        address collection,
        uint256[] memory tokenIds,
        bytes32 request,
        TrustusPacket calldata packet
    ) internal view verifyPacket(request, packet) returns (uint256) {
        DataTypes.Assets721Price memory priceParams = abi.decode(
            packet.payload,
            (DataTypes.Assets721Price)
        );
    ...
    function _getTokens1155ETHPrice(
        address collection,
        uint256[] memory tokenIds,
        uint256[] memory tokenAmounts,
        bytes32 request,
        ...
        DataTypes.Assets1155Price memory priceParams = abi.decode(
            packet.payload,
            (DataTypes.Assets1155Price)
        );
        // Make sure the request is for the right token
        require(
            collection == priceParams.collection,
        ...
        // Make sure the tokens amounts coincide
        require(
            tokenAmounts.length == priceParams.tokenAmounts.length,
            "NFTO:GTEP:AMOUNTS_LENGTH_MISMATCH"
        );
        for (uint i = 0; i < tokenIds.length; i++) {
            require(
                tokenIds[i] == priceParams.tokenIds[i],
            ...
            require(
                tokenAmounts[i] == priceParams.tokenAmounts[i],
                "NFTO:GTEP:AMOUNT_MISMATCH"
            );
        }
