...
describe("Interest Rate fuzzing", function () {
  load.loadTest(false);
  ...
  before(async function () {
    ...
    for (var j = 0; j < nLoans; j++) {
      const mintNFTTx = await testERC721.mint(owner.address);
      await mintNFTTx.wait();

      // Approve the lending pool to take the NFT
      const approveTx = await testERC721.approve(lendingMarket.address, j);
