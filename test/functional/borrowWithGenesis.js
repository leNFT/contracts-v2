...
describe("Borrow using Genesis NFT", function () {
  ...
  it("Create NFT asset 1", async function () {
    ...
    expect(await testERC721.ownerOf(tokenID1)).to.equal(owner.address);
  });
  it("Create NFT asset 2", async function () {
    ...
    // Find if the NFT was minted accordingly
    expect(await testERC721.ownerOf(tokenID2)).to.equal(owner.address);
  });
  it("Deposit underlying to the lending pool", async function () {
    ...
    const depositTx = await wethGateway.depositLendingPool(
      await lendingMarket.getLendingPool(testERC721.address, weth.address),
  ...
  it("Borrow using NFT asset as collateral", async function () {
    ...
    const approveNftTx1 = await testERC721.approve(
      wethGateway.address,
      tokenID1
    );
    await approveNftTx1.wait();
    const approveNftTx2 = await testERC721.approve(
      wethGateway.address,
      tokenID2
    );
    ...
    const borrowTx = await wethGateway.borrow721(
    ...
    expect(await testERC721.ownerOf(tokenID1)).to.equal(lendingMarket.address);
    expect(await testERC721.ownerOf(tokenID2)).to.equal(lendingMarket.address);
  ...
  it("Throw an error if we try to use the genesis NFT again", async function () {
    ...
    const approveNftTx = await testERC721.approve(
      wethGateway.address,
      tokenID3
    );
