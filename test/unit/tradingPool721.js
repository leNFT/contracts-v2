...
describe("TradingPool721", function () {
  load.loadTest(false);
  ...
  it("Should get the correct token URI", async function () {
    ...
    const createPoolTx = await tradingPool721Factory.create(
      testERC721.address,
      weth.address
    );
    createPoolTx.wait();
    ...
    const tradingPool = await ethers.getContractAt(
      "TradingPool721",
      await tradingPoolRegistry.getTradingPool(testERC721.address, weth.address)
    );
    ...
    const tokenURIMetadata = await liquidityPair721Metadata.tokenURI(
      tradingPool.address,
      0
    );
  ...
  it("Should be able to sell one token", async function () {
    ...
    // Should now own both tokens
    expect(await testERC721.ownerOf(0)).to.equal(tradingPool.address);
  ...
  it("Should be able to sell multiple tokens", async function () {
    ...
    // Should now own both tokens
    expect(await testERC721.ownerOf(0)).to.equal(tradingPool.address);
    expect(await testERC721.ownerOf(1)).to.equal(tradingPool.address);
