...
describe("LoanCenter", function () {
  ...
  it("Should set the risk params for a collection", async function () {
    ...
    expect(await loanCenter.getCollectionMaxLTV(testERC721.address)).to.equal(
      newMaxLTV
    );
    expect(
      await loanCenter.getCollectionLiquidationThreshold(testERC721.address)
  ...
  it("Should get the correct amount of loans", async function () {
    ...
    const mintTx = await testERC721.mint(owner.address);
    await mintTx.wait();
    const approveNftTx = await testERC721.approve(lendingMarket.address, 0);
    ...
    const borrowTx = await lendingMarket.borrow721(
      owner.address,
      weth.address,
  ...
  it("Should get the correct loan after borrowing", async function () {
    ...
    expect(loan.tokenIds).to.deep.equal([BigNumber.from(0)]);
    expect(loan.asset).to.equal(testERC721.address);
    expect(await loanCenter.getLoanCollectionAddress(0)).to.equal(
      testERC721.address
    );
    expect(await loanCenter.getNFTLoanId(testERC721.address, 0)).to.equal(
      BigNumber.from(0)
    );
