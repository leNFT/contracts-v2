...
describe("LiquidityPair721Metadata", function () {
  load.loadTest(false);
  ...
  it("Should get a valid JSON token URI for a certain liquidity pair", async function () {
    ...
    const approveNFTTx = await testERC721.setApprovalForAll(poolAddress, true);
    ...
    const tokenURI = await liquidityPair721Metadata.tokenURI(poolAddress, 0);
    ...
    await expect(
      liquidityPair721Metadata.tokenURI(poolAddress, 1)
  ...
  it("Should get a valid SVG", async function () {
    ...
    const svg = await liquidityPair721Metadata.svg(poolAddress, 0);
