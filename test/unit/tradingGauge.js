...
describe("TradingGauge", () => {
  ...
  it("Should deposit into a trading gauge", async function () {
    ...
    const approveNFTTx = await testERC721.approve(tradingPool.address, 0);
  ...
  it("Should batch withdraw from  a trading gauge", async function () {
    ...
    await approveTx.wait();
    // Mint two new NFT
    const mintTx1 = await testERC721.mint(owner.address);
    await mintTx1.wait();
    const mintTx2 = await testERC721.mint(owner.address);
    await mintTx2.wait();
    // Approve the trading pool to spend the NFT
    const approveNFTTx = await testERC721.approve(tradingPool.address, 0);
    await approveNFTTx.wait();
    const approveNFTTx2 = await testERC721.approve(tradingPool.address, 1);
