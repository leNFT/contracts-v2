...
describe("ExponentialPriceCurve", function () {
  ...
  it("Should calculate the correct price after multiple buys", async function () {
    const amount = 3;
    const price = ethers.utils.parseEther("1");
    const delta = 500;
    const expectedPrice = ethers.utils.parseEther("1.157625");

    const newPrice = await exponentialPriceCurve.priceAfterMultipleBuys(
      amount,
      price,
      delta
    );
  ...
  it("Should calculate the correct buy price for multiple buys", async function () {
    const amount = 3;
    const price = ethers.utils.parseEther("0.01");
    const delta = 500;
    const expectedPrice = ethers.utils.parseEther("0.031525");

    const newPrice = await exponentialPriceCurve.buyPriceSum(
      amount,
      price,
      delta
    );
  ...
  it("Should calculate the correct price after multiple sells", async function () {
    const amount = 3;
    const price = ethers.utils.parseEther("1");
    const delta = 500;
    const expectedPrice = ethers.utils.parseEther("0.863837598531476082");

    const newPrice = await exponentialPriceCurve.priceAfterMultipleSells(
      amount,
      price,
      delta,
      0
  ...
  it("Should calculate the correct sell price for multiple sells", async function () {
    const amount = 3;
    const price = ethers.utils.parseEther("1");
    const delta = 500;
    const expectedPrice = ethers.utils.parseEther("2.859410430839002267");

    const newPrice = await exponentialPriceCurve.sellPriceSum(
      amount,
      price,
      delta
    );
